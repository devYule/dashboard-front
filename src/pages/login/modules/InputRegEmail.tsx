...
export default function InputRegEmail({ servKey }: { servKey: string }) {
  ...
  const emailInputStyle = {
    borderColor:
      emailChecker && enabled ? "black" : placeholder ? "black" : "red",
  };
  const emailBtnStyle = {
    opacity:
      emailInputStyle.borderColor === "black"
        ? email.length === 0
          ? 0.2
          : 1
        : 0.2,
  ...
  const codeBtnStyle = {
    opacity:
      codeInputStyle.borderColor === "black"
        ? code.length === 0
          ? 0.2
          : 1
        : 0.2,
    ...
    <>
      {!isValidating ? (
        <div className={mailContainerClassName}>
          ...
          <InputLineMail
            className={mailPlaceholderClass}
            style={emailInputStyle}
            type="mail"
            value={email}
            onChange={(e) => onChange(e, 7, 35, setEmail, setEmailChecker)}
        ...
        <nav className={mailContainerClassName}>
          ...
          <InputLineMail
            className={codePlaceholderClass}
            style={codeInputStyle}
            type="text"
            value={code}
            onChange={(e) => onChange(e, 6, 6, setCode, setCodeChecker)}
            onKeyDown={(e) => onKeyDown(e)}
            autoFocus
            placeholder="인증코드 ( 6자 )"
            disabled={!isValidating}
  ...
  async function onBtnClick() {
    ...
    if (!isValidating) {
      ...
      await axiosInstance
        .post("/api/user/mail", { key: servKey, mail: email })
        .then((res) => {
          if (res.data.code > 0) {
    ...
    await axiosInstance
      .post(`/api/user/mail/check`, { key: servKey, code: code })
      .then((res) => {
        if (res.data.code > 0) {
