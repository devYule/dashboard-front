...
export default function MypageConatiner() {
  ...
  // infos
  const [userInfos, setUserInfos] = useState<UserInfosRef>({
    pic: "",
    nick: "",
    mail: "",
    sites: [],
  ...
  const newPwdLengthChecker =
    inputPwd.length === 0
      ...
      : true;
  const isCorrect =
    checkPwd.length === 0 ? true : inputPwd === checkPwd ? true : false;
            ...
            {modalType === ModalType.CHANGE_PASSWORD ? (
              <>
                ...
                <input
                  ...
                  style={{ borderColor: curPwdLengthChecker ? "black" : "red" }}
                ></input>
                <input
                  id="nextPwd"
                  type="password"
                  placeholder="변경할 비밀번호 ( 6 ~ 20자 사이 )"
                  onChange={(e) => setInputPwd(e.target.value)}
                  style={{ borderColor: newPwdLengthChecker ? "black" : "red" }}
                ></input>
                <input
                  id="nextPwdAgain"
                  type="password"
                  placeholder="변경할 비밀번호 확인 ( 6 ~ 20자 사이 )"
                ...
                <section className="btns">
                  ...
                  <button className="btn pointer" onClick={onCancelBtnClick}>
                    <p>취소</p>
                  </button>
                </section>
              </>
              ...
              <>
                <p>{`정말 ${
                  modalType === ModalType.RESET_WIDGET
                    ? "모든 위젯을 초기화"
                    : "탈퇴"
                } 하시겠습니까?`}</p>
                <section className="btns">
                  <button className="btn pointer" onClick={onSubmit}>
                    <p>
                      {modalType === ModalType.RESET_WIDGET ? "초기화" : "탈퇴"}
                    </p>
                  </button>
                  <button className="btn pointer" onClick={onCancelBtnClick}>
                    <p>취소</p>
  ...
  async function changePw() {
    await axiosInstance
      .put("/api/mypage/pw", { prevPw: curPwd, pw: inputPwd })
      .then((res) => {
  ...
  async function resetWidget() {
    await axiosInstance.delete("/api/widget/all").then((res) => {
      console.log("code", res.data.code);
      console.log("value", res.data.value);
  ...
  async function withdraw() {
    await axiosInstance.delete("/api/mypage").then((res) => {
      console.log("code", res.data.code);
      console.log("value", res.data.value);
